...
func DeleteCommentHandler(c *gin.Context) {
	var requestBody struct {
		ID     primitive.ObjectID `json:"_id"`//required
	...
	res, err := db.CommentCollection.DeleteOne(context.Background(), filter)
	if err != nil {
	...
	postRes, err := db.PostCollection.UpdateOne(context.Background(), filter, update)
	if err != nil {
		c.IndentedJSON(http.StatusInternalServerError, gin.H{"status": "error", "message": err})
		return
	}
